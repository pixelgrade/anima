...
var object_assign = __webpack_require__(33);
...
(function ($, window) {
	...
	function initialize() {
		...
		var HeroCollection = heroElementsArray.map(function (element) {
			return new components_hero(element);
		});

		// initialize header
		var firstHero = heroElementsArray[0];
		var headerElement = $('.site-header').get(0);
		var header = new components_header(headerElement);

		var navbar = new components_navbar();

		// initialize hero
		var promo = new promo_bar(document.querySelectorAll('.promo-bar .novablocks-announcement-bar'), {
			onUpdate: function onUpdate(promo) {
				...
				HeroCollection.forEach(function (hero) {
					...
					$(hero.element).find('.novablocks-hero__parallax').each(function (i, obj) {
						reloadRellax(obj);
					});
				});
			}
