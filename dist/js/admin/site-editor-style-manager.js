...
(function () {
  ...
  function injectStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = '.edit-site-styles.' + DISABLED_CLASS + ' > :not(.' + NOTICE_CLASS + '){display:none!important;}' + '.edit-site-styles.' + DISABLED_CLASS + '{padding:24px;}' + '.' + NOTICE_CLASS + '{display:grid;gap:16px;}' + '.' + NOTICE_CLASS + '__intro .components-card__body{display:grid;gap:16px;}' + '.' + NOTICE_CLASS + '__eyebrow,.' + NOTICE_CLASS + '__resources-eyebrow{margin:0;font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;}' + '.' + NOTICE_CLASS + '__title,.' + NOTICE_CLASS + '__resource-title{margin:0;}' + '.' + NOTICE_CLASS + '__description,.' + NOTICE_CLASS + '__resource-description{margin:0;}' + '.' + NOTICE_CLASS + '__actions{display:grid;gap:12px;}' + '.' + NOTICE_CLASS + '__resources-section{display:grid;gap:12px;}' + '.' + NOTICE_CLASS + '__resources{display:grid;gap:16px;grid-template-columns:repeat(auto-fit,minmax(min(220px,100%),1fr));}' + '.' + NOTICE_CLASS + '__resource .components-card__body{display:grid;gap:12px;align-content:start;}' + '.' + NOTICE_CLASS + ' .components-button{width:100%;box-sizing:border-box;justify-content:center;}' + '@media (max-width: 781px){.edit-site-styles.' + DISABLED_CLASS + '{padding:16px;}}';
