...
* Fix stacked collection cards with a fixed aspect ratio collapsing to empty colored boxes on the frontend when the Pile collection hover effect is active.
...
* Add WordPress 7.1 compatibility by refreshing shipped block markup and keeping frontend runtimes out of the iframed editor canvas.
* Keep heading emphasis typography and gradients aligned with the configured accent font while retaining the intended Headline kicker sizing.
* Render the Site Frame close icon consistently through its CSS mask.
