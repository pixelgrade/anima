<?php
/**
 * Handle Style Manager extra functionality needed to make things work smoothly.
 */
...
// This function should come from Style Manager, but we need to do our best to make things happen
if ( ! function_exists( 'pixelgrade_option' ) ) {
	 ...
	 *                                    When false, we will let the option's default set value (in the Style Manager settings) kick in first, then our $default.
	...
	function pixelgrade_option( string $option_id, $default = null, bool $force_given_default = false ) {
		if ( function_exists( '\Pixelgrade\StyleManager\get_option_details_all()' ) ) {
			...
			if ( ! empty( $options_config ) && ! empty( $options_config[ $option_id ] ) ) {
				...
				if ( $default !== null && false === $force_given_default ) {
					// We will not pass the received $default here so Style Manager will fallback on the option's default value, if set
					$sm_value = \Pixelgrade\StyleManager\get_option( $option_id );

					// We only fallback on the $default if none was given from Style Manager
					if ( null === $sm_value ) {
						return $default;
					}
				} else {
					$sm_value = \Pixelgrade\StyleManager\get_option( $option_id, $default );
				}
		...
		// We don't have Style Manager present, or Style Manager doesn't "know" about this option ID, so we need to retrieve the option value the hard way.
		$option_value = null;
